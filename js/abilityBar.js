// Ability Bar Logic
// Dynamische Leiste unten: Slots werden anhand (Character + Waffe) gefüllt.
// Erweiterung: Neue Fähigkeiten einfach zu mappingTable hinzufügen.

(function(){
  // Prevent double-injection if script loaded twice
  ...
  // Mapping: character -> weapon -> slot -> ability descriptor
  // descriptor: { name, iconText, cdGetter(), activeGetter(), readyGetter(), placeholder }
  // Altes mappingTable entfernt – dynamik jetzt über abilities.registry & bindings

  function el(tag, cls){ const e=document.createElement(tag); if(cls) e.className=cls; return e; }
  function resolveAbilityForSlot(def){
    const ab = window.abilities; if(!ab) return null;
    // Slot mit key -> suche binding
    if(def.key){ const b = ab.getBindingByKey(def.key); if(b) return ab.registry[b.ability]; }
    // fallback: slot name equals ability id
  ...
  function build(){
    ...
    // Full rebuild: clear but keep a lightweight placeholder to avoid layout flicker
    while(host.firstChild) host.removeChild(host.firstChild);
    // Lock Toggle Button (rechts angehängt)
    const lockBtn = document.createElement('button');
    lockBtn.id='abilityBarLockBtn';
    lockBtn.textContent = window.abilities && window.abilities._locked ? '🔒' : '🔓';
    ...
    lockBtn.addEventListener('click',()=>{ window.abilities._locked=!window.abilities._locked; lockBtn.textContent=window.abilities._locked?'🔒':'🔓'; document.getElementById('abilityBar')?.classList.toggle('locked', !!window.abilities._locked); });
  ...
  // Soul Counter Badge für Meteor (R)
  let soulBadge = null;
  ...
  // (Key label removed per user request - only single label desired)
  const overlay = el('div','cd-overlay'); // dark radial sector overlay
  icon.appendChild(overlay);
      const num = el('div','cd-num'); // numeric countdown
  icon.appendChild(num);
  const stacks = el('div','ab-stacks'); // stack/charges badge
  icon.appendChild(stacks);
    // Taste/Key unter dem Icon anzeigen, falls vorhanden
    if(def.key){
  ...
  keyLabel.style.fontSize = '0.58em';
    ...
    keyLabel.style.letterSpacing = '0.5px';
      ...
      // Weapon level badge (nur für basic)
      if(def.slot==='basic'){
  ...
  if(lockedVisual && (ability.placeholder || def.slot==='reserve' || def.slot==='passive')){ const lock = el('div','lock'); lock.textContent='🔒'; slot.appendChild(lock); }
  ...
  function update(){
    ...
    [...host.children].forEach(slot=>{
      ...
      if(ability && ability.showCondition && !ability.showCondition()){ // wurde unsichtbar -> rebuild anstoßen
        // Mark zum späteren Rebuild (verhindert massenhaft rebuilds im selben Frame)
      ...
      // Variant refresh (Name/Icon) wenn vorhanden
      ...
      // Weapon Level Badge aktualisieren
      ...
      // remaining fraction (1 = frisch begonnen, 0 = fertig)
  ...
  function enableDrag(host){
    ...
    host.querySelectorAll('.ab-slot').forEach(slot=>{
      ...
      if(!key) return; // passive/reserve nicht verschiebbar vorerst
  ...
  function showTooltip(slot, ability, ev){
    ...
    // Versuche variant() für kontextabhängigen Text (z.B. unterschiedliche Ults pro Waffe)
  ...
  // Rebuild hook erweitern um Tooltips
  // Hook rebuild safely (falls vorher nicht definiert)
  // Robust rebuild wrapper with guard & queue
  ...
  // Style helpers for drag visuals
  ...
  // initial build when DOM ready & script.js loaded
  ...
  // Rebuild bei Wechsel von Character oder Waffe erkennen
  ...
  (function watchCharWeapon(){
    ...
    if(char!==lastChar || wId!==lastWeapon){
      ...
      // Debounce: schedule once per frame via requestAnimationFrame (already inside rAF loop)
  ...
  // Expose small helper for console (nicht überschreiben wenn Wrapper schon gesetzt)
  if(!window.__abConsoleHelper){
    window.__abConsoleHelper = true;
    window.rebuildAbilityBarRaw = build; // direkter Build ohne Tooltips Wrapper
  }
})();
